...
<xsl:stylesheet version="1.0"
	...
	extension-element-prefixes="redirect java util utils2">
	<xsl:param name="file"/>
...
-->
					...
					<xsl:if test="Script/Version[(@mode=$versionMode) or (@Baseline_mode=$versionMode)]/VersionFields/descendant::*[@value!='' or @Baseline_value!='']">
						<h3>
							<xsl:text>Snapshot of non-empty fields before commit</xsl:text>
						</h3>

						<xsl:for-each select="Script/Version[(@mode=$versionMode) or (@Baseline_mode=$versionMode)]/VersionFields">
							<table width="100%" border="2">
								<thead>
									<tr>
								...
								<tbody>
									<xsl:apply-templates select=".//*[@value!='' or @Baseline_value!='']" mode="VersionFields"/>
								</tbody>
							</table>
						</xsl:for-each>
					</xsl:if>
					...
					<xsl:if test="($versionMode!='S') and Script/Version[(@mode='S') or (@Baseline_mode='S')]/VersionFields/descendant::*[@value!='' or @Baseline_value!='']">
						<h3>
							<xsl:text>Snapshot of non-empty fields after commit</xsl:text>
						</h3>
						<xsl:for-each select="Script/Version[(@mode='S') or (@Baseline_mode='S')]/VersionFields">
							<table width="100%" border="2">
								<thead>
									<tr>
										<th colspan="4" align="center">
								...
								<tbody>
									<xsl:apply-templates select=".//*[@Baseline_value!='' or @value!='']" mode="VersionFields"/>
								</tbody>
							</table>
						</xsl:for-each>
					</xsl:if>
...
-->
					...
					<xsl:if test="Script/Version[(@mode=ancestor::Sequence/Inputs/InputMessage/FieldValues/@Mode) or (@Baseline_mode=ancestor::Sequence/Inputs/InputMessage/FieldValues/@Baseline_Mode)]/VersionFields/descendant::*[@value!='' or @Baseline_value!='']">

						<h3>
							<xsl:text>Snapshot of non-empty fields before commit</xsl:text>
						</h3>

						<xsl:for-each select="Script/Version[(@mode=ancestor::Sequence/Inputs/InputMessage/FieldValues/@Mode) or (@Baseline_mode=ancestor::Sequence/Inputs/InputMessage/FieldValues/@Baseline_Mode)]/VersionFields">
					...
					<xsl:if test="Script/Version[(@mode='S') or (@Baseline_mode='S')]/VersionFields/descendant::*[@value!='' or @Baseline_value!=''] and (ancestor::Sequence/Inputs/InputMessage/FieldValues/@Mode!='S' or ancestor::Sequence/Inputs/InputMessage/FieldValues/@Baseline_Mode!='S')">
						<h3>
							<xsl:text>Snapshot of non-empty fields after commit</xsl:text>
						</h3>
						<xsl:for-each select="Script/Version[(@mode='S') or (@Baseline_mode='S')]/VersionFields">
	...
	<xsl:template match="RECEIVED" mode="version">
			...
			<xsl:choose>
				<xsl:when test="contains($text_html, '&lt;div class=&quot;dprocessing&quot; id=')">
					...
					<xsl:call-template name="replace-string">
						<xsl:with-param name="text" select="substring-before(substring-after($text_html,'&lt;div class=&quot;dprocessing&quot; id=&quot;processingPage&quot;&gt;'),'&lt;form name=&quot;enquiry&quot; method=&quot;POST&quot; action=&quot;BrowserServlet&quot; id=&quot;enquiry&quot;&gt;')"/>
				...
				<xsl:otherwise>
					<xsl:call-template name="replace-string">
						<xsl:with-param name="text" select="substring-before(substring-after($text_html,'&lt;div class=&quot;dheader&quot; id=&quot;divHeader&quot;&gt;'),'&lt;form name=&quot;enquiry&quot; method=&quot;POST&quot; action=&quot;BrowserServlet&quot; id=&quot;enquiry&quot;&gt;')"/>
						<xsl:with-param name="replace" select="$appt"/>
						<xsl:with-param name="with" select="$apps"/>
					</xsl:call-template>
				</xsl:otherwise>
		...
		<xsl:call-template name="putRecHtml">
			<xsl:with-param name="html" select="$newText5"/>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template match="RECEIVED" mode="enquiry">
		...
		<xsl:variable name="newText">
			<xsl:call-template name="replace-string">
				<xsl:with-param name="text" select="substring-before(substring-after(substring-after($text_html,'class=&quot;denqresponse'),'&gt;'),'&lt;form name=&quot;enqsel&quot;')"/>
				<xsl:with-param name="replace" select="$tabt"/>
				<xsl:with-param name="with" select="$tabs"/>
			</xsl:call-template>
		</xsl:variable>
		...
		<xsl:call-template name="putRecHtml">
			<xsl:with-param name="html" select="$newText4"/>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template match="/">
		...
		<xsl:variable name="pdfCss" select="//@pdfCss"/>
		...
		<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
			<head>
				...
				<link rel="stylesheet" type="text/css" href="../plaf/style/default/custom.css"/>
				...
				<xsl:when test="count(./Testcase)=1 or contains(@Value,'#ENTER') or contains(@Baseline_Value,'#ENTER')">
					<td>
						<xsl:if test="substring-after(substring-before(current()/@Text,'.)'),'(')=''">
							<xsl:text>'</xsl:text>
							<xsl:value-of select="ancestor::Version/VersionFields/descendant::*[contains(current()/@Text,name())]/@value"/>
							<xsl:text>'</xsl:text>
						</xsl:if>
						<xsl:if test="substring-after(substring-before(current()/@Text,'.)'),'(')!=''">
							<xsl:text>'</xsl:text>
							<xsl:value-of select="ancestor::Version/VersionFields/descendant::*[contains(current()/@Text,name())][position()=number(substring-before(substring-after(current()/@Text,'('),'.'))]/@value"/>
							<xsl:text>'</xsl:text>
						</xsl:if>
					...
					<xsl:if test="$isCompare=1">
						<td>
							<xsl:if test="substring-after(substring-before(current()/@Baseline_Text,'.)'),'(')=''">
								<xsl:text>'</xsl:text>
								<xsl:value-of select="ancestor::Version/VersionFields/descendant::*[contains(current()/@Baseline_Text,name())]/@Baseline_value"/>
								<xsl:text>'</xsl:text>
							</xsl:if>
							<xsl:if test="substring-after(substring-before(current()/@Baseline_Text,'.)'),'(')!=''">
								<xsl:text>'</xsl:text>
								<xsl:value-of select="ancestor::Version/VersionFields/descendant::*[contains(current()/@Baseline_Text,name())][position()=number(substring-before(substring-after(current()/@Baseline_Text,'('),'.'))]/@Baseline_value"/>
								<xsl:text>'</xsl:text>
							</xsl:if>
